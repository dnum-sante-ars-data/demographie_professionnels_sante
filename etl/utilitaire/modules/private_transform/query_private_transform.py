...
SUBSTR( DATE_DE_DEBUT_ACTIVITE, 7, 4 ) || "-" || SUBSTR(DATE_DE_DEBUT_ACTIVITE,4,2) || "-" || SUBSTR(DATE_DE_DEBUT_ACTIVITE,1,2) AS DATE_DE_DEBUT_ACTIVITE ,
SUBSTR( DATE_DE_FIN_ACTIVITE, 7, 4 ) || "-" || SUBSTR(DATE_DE_FIN_ACTIVITE,4,2) || "-" || SUBSTR(DATE_DE_FIN_ACTIVITE,1,2) AS DATE_DE_FIN_ACTIVITE
FROM ACTIVITE),
SAVOIRFAIRES AS (SELECT
IDENTIFIANT_PP ,
LIBELLE_SAVOIR_FAIRE ,
LIBELLE_TYPE_SAVOIR_FAIRE ,
SUBSTR( DATE_RECONNAISSANCE_SAVOIR_FAIRE, 7, 4 ) || "-" || SUBSTR( DATE_RECONNAISSANCE_SAVOIR_FAIRE,4,2) || "-" || SUBSTR( DATE_RECONNAISSANCE_SAVOIR_FAIRE,1,2) AS DATE_RECONNAISSANCE_SAVOIR_FAIRE ,
SUBSTR (DATE_ABANDON_SAVOIR_FAIRE, 7, 4 ) || "-" || SUBSTR( DATE_ABANDON_SAVOIR_FAIRE,4,2) || "-" || SUBSTR( DATE_ABANDON_SAVOIR_FAIRE,1,2) AS DATE_ABANDON_SAVOIR_FAIRE
FROM SAVOIRFAIRE)
SELECT 
REGION_CD,
REGION_LB,
...
LIBELLE_PROFESSION,
LIBELLE_CATEGORIE_PROFESSIONNELLE,
COALESCE(DATE_DE_DEBUT_ACTIVITE, "9999-12-31") AS DATE_DE_DEBUT_ACTIVITE,
COALESCE(DATE_DE_FIN_ACTIVITE, "9999-12-31") AS DATE_DE_FIN_ACTIVITE,
IIF(COALESCE(DATE_DE_FIN_ACTIVITE, "9999-12-31")>=STRFTIME('%Y-%m-%d','now'),"en cours", "historique") AS STATUT_ACTIVITE,
LIBELLE_SAVOIR_FAIRE,
LIBELLE_TYPE_SAVOIR_FAIRE,
COALESCE(DATE_RECONNAISSANCE_SAVOIR_FAIRE, "9999-12-31") AS DATE_RECONNAISSANCE_SAVOIR_FAIRE,
COALESCE(DATE_ABANDON_SAVOIR_FAIRE, "9999-12-31") AS DATE_ABANDON_SAVOIR_FAIRE
FROM 
	(SELECT 
        DISTINCT a.IDENTIFIANT_DE_L_ACTIVITE ,
	ir.REG AS REGION_CD ,
    ...
    rad.CODE_DEPARTEMENT_INSCRIPTION AS DEPARTEMENT_CD ,
    rad.LIBELLE_DEPARTEMENT_INSCRIPTION  AS DEPARTEMENT_LB ,
    tgc.TVS_CODE AS TVS_CD ,
	tgc.TVS_LIB AS TVS_LB ,
    (IIF(ca.CODE_COMMUNE_COORD_ACTIVITE IS NOT NULL, ca.CODE_COMMUNE_COORD_ACTIVITE , 
    IIF(cs.CODE_COMMUNE_COORD_STRUCTURE IS NOT NULL, cs.CODE_COMMUNE_COORD_STRUCTURE ,
    cc.CODE_COMMUNE_COORD_CORRESPONDANCE))) AS COMMUNE_CD ,
    ic.LIBELLE AS COMMUNE_LB ,
	a.IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE ,
	a.IDENTIFIANT_PP ,
	p.NOM_D_USAGE ,
	p.PRENOM_D_USAGE ,
	...
	FROM ACTIVITES a
	LEFT JOIN SAVOIRFAIRES sf ON 
	a.IDENTIFIANT_PP = sf.IDENTIFIANT_PP 
	AND (sf.DATE_RECONNAISSANCE_SAVOIR_FAIRE <= COALESCE(a.DATE_DE_FIN_ACTIVITE, "9999-12-31")
	AND (sf.DATE_ABANDON_SAVOIR_FAIRE IS NULL OR sf.DATE_ABANDON_SAVOIR_FAIRE >= a.DATE_DE_DEBUT_ACTIVITE)
	AND (sf.DATE_RECONNAISSANCE_SAVOIR_FAIRE BETWEEN a.DATE_DE_DEBUT_ACTIVITE AND COALESCE(a.DATE_DE_FIN_ACTIVITE, "9999-12-31")
	     OR sf.DATE_ABANDON_SAVOIR_FAIRE BETWEEN a.DATE_DE_DEBUT_ACTIVITE AND COALESCE(a.DATE_DE_FIN_ACTIVITE, "9999-12-31")
	     OR sf.DATE_RECONNAISSANCE_SAVOIR_FAIRE <= a.DATE_DE_DEBUT_ACTIVITE AND sf.DATE_ABANDON_SAVOIR_FAIRE >= COALESCE(a.DATE_DE_FIN_ACTIVITE, "9999-12-31")))
	LEFT JOIN COORDACT ca ON a.IDENTIFIANT_DE_L_ACTIVITE=ca.IDENTIFIANT_DE_L_ACTIVITE
    LEFT JOIN STRUCTURE s ON a.IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE=s.IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE   
    LEFT JOIN COORDSTRUCT cs ON a.IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE=cs.IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE
    LEFT JOIN REFERAE ra ON a.IDENTIFIANT_PP=ra.IDENTIFIANT_PP
    ...
    *
    ...
    (IIF(ca.CODE_COMMUNE_COORD_ACTIVITE IS NOT NULL, ca.CODE_COMMUNE_COORD_ACTIVITE , 
    IIF(cs.CODE_COMMUNE_COORD_STRUCTURE IS NOT NULL, cs.CODE_COMMUNE_COORD_STRUCTURE ,
    cc.CODE_COMMUNE_COORD_CORRESPONDANCE))) = ic.COM 
    -- identification TVS
    LEFT JOIN (
    SELECT 
	IIF(LENGTH(COM_CODE)= 4, '0' || COM_CODE, COM_CODE) AS COM_CODE ,
	IIF(LENGTH(TVS_CODE)= 4, '0' || TVS_CODE, TVS_CODE) AS TVS_CODE ,
	TVS_LIB 
	FROM T_GEO_COM 
	WHERE COM_TYPE = 'COM'
    ) tgc ON 
    (IIF(ca.CODE_COMMUNE_COORD_ACTIVITE IS NOT NULL, ca.CODE_COMMUNE_COORD_ACTIVITE , 
    IIF(cs.CODE_COMMUNE_COORD_STRUCTURE IS NOT NULL, cs.CODE_COMMUNE_COORD_STRUCTURE ,
    cc.CODE_COMMUNE_COORD_CORRESPONDANCE))) = tgc.COM_CODE
    LEFT JOIN INSEE_DEPARTEMENT id ON 
    IIF( 
    -- condition : code département n'est pas 97 ou 98
    (IIF(ca.CODE_COMMUNE_COORD_ACTIVITE IS NOT NULL, SUBSTR(ca.CODE_COMMUNE_COORD_ACTIVITE, 1, 2), 
...
def query_select_last_activites_vs_savoirfaire_sql():
    query = """
	WITH 
	ACTIVITES AS (SELECT
		ACTIVITE.IDENTIFIANT_PP ,
		ACTIVITE.IDENTIFIANT_DE_L_ACTIVITE ,
		ACTIVITE.IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE ,
		ACTIVITE.LIBELLE_FONCTION ,
		ACTIVITE.LIBELLE_MODE_EXERCICE ,
		COALESCE(SUBSTR( DATE_DE_FIN_ACTIVITE, 7, 4 ) || "-" || SUBSTR(DATE_DE_FIN_ACTIVITE,4,2) || "-" || SUBSTR(DATE_DE_FIN_ACTIVITE,1,2), "9999-12-31") AS DATE_DE_FIN_ACTIVITE ,
		ACTIVITE.LIBELLE_PROFESSION ,
		ACTIVITE.LIBELLE_CATEGORIE_PROFESSIONNELLE 
		FROM ACTIVITE) ,
	MAX_ACTIVITES AS (SELECT
		IDENTIFIANT_PP ,
		MAX(DATE_DE_FIN_ACTIVITE) AS DATE_DE_FIN_ACTIVITE ,
		IDENTIFIANT_DE_L_ACTIVITE ,
		IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE ,
		LIBELLE_FONCTION ,
		LIBELLE_MODE_EXERCICE ,
		...
		LIBELLE_CATEGORIE_PROFESSIONNELLE 
		FROM ACTIVITES
		GROUP BY IDENTIFIANT_PP) ,
	SAVOIRFAIRES AS (SELECT 
		IDENTIFIANT_PP ,
		LIBELLE_SAVOIR_FAIRE ,
		LIBELLE_TYPE_SAVOIR_FAIRE ,
		COALESCE(SUBSTR (DATE_ABANDON_SAVOIR_FAIRE, 7, 4 ) || "-" || SUBSTR( DATE_ABANDON_SAVOIR_FAIRE,4,2) || "-" || SUBSTR( DATE_ABANDON_SAVOIR_FAIRE,1,2), "9999-12-31") AS DATE_ABANDON_SAVOIR_FAIRE
		FROM SAVOIRFAIRE) ,
		MAX_SAVOIRFAIRES AS (SELECT
		IDENTIFIANT_PP ,
		MAX(DATE_ABANDON_SAVOIR_FAIRE) AS DATE_ABANDON_SAVOIR_FAIRE ,
		LIBELLE_SAVOIR_FAIRE ,
		LIBELLE_TYPE_SAVOIR_FAIRE
		FROM SAVOIRFAIRES
		GROUP BY IDENTIFIANT_PP) ,
	MAX_ACT_VS_SF AS (SELECT 
		MAX_ACTIVITES.IDENTIFIANT_PP ,
		DATE_DE_FIN_ACTIVITE ,
		IDENTIFIANT_DE_L_ACTIVITE ,
		IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE ,
		LIBELLE_FONCTION ,
		...
		LIBELLE_TYPE_SAVOIR_FAIRE 
		FROM MAX_ACTIVITES
		LEFT JOIN MAX_SAVOIRFAIRES ON MAX_ACTIVITES.IDENTIFIANT_PP = MAX_SAVOIRFAIRES.IDENTIFIANT_PP)
	SELECT 
	MAX_ACT_VS_SF.IDENTIFIANT_PP ,
	NOM_D_USAGE ,
	PRENOM_D_USAGE ,
	LIBELLE_PROFESSION ,
	LIBELLE_CATEGORIE_PROFESSIONNELLE ,
	...
	LIBELLE_MODE_EXERCICE ,
	LIBELLE_TYPE_SAVOIR_FAIRE ,
	MAX_ACT_VS_SF.IDENTIFIANT_DE_L_ACTIVITE ,
	IDENTIFIANT_TECHNIQUE_DE_LA_STRUCTURE
	FROM MAX_ACT_VS_SF
	LEFT JOIN (SELECT
		IDENTIFIANT_PP ,
		NOM_D_USAGE ,
		PRENOM_D_USAGE 
		FROM PERSONNE) PERSONNE_LJ ON MAX_ACT_VS_SF.IDENTIFIANT_PP = PERSONNE_LJ.IDENTIFIANT_PP
	LEFT JOIN (SELECT 
		IDENTIFIANT_DE_L_ACTIVITE ,
		CODE_POSTAL_COORD_ACTIVITE ,
		CODE_COMMUNE_COORD_ACTIVITE ,
		LIBELLE_COMMUNE_COORD_ACTIVITE 
		FROM COORDACT) COORDACT_LJ ON MAX_ACT_VS_SF.IDENTIFIANT_DE_L_ACTIVITE = COORDACT_LJ.IDENTIFIANT_DE_L_ACTIVITE
        """
