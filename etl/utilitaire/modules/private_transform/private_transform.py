...
def transform_export(filepath_activites, filepath_personnes, filepath_activites_vs_savoirfaire, filepath_last_activites_vs_savoirfaire, database="database", verbose = True) :
    """
    ...
    if os.path.exists(filepath_last_activites_vs_savoirfaire):
        os.remove(filepath_last_activites_vs_savoirfaire)
    ...
    # transformation et export des données dernières activités vs derniers savoir-faire query_select_last_activites_vs_savoirfaire_sql
    print(" ")
    print(" - - - Transformation et export des données dernières activites vs derniers savoir-faire...")

    select_last_activites_vs_savoirfaire_sql = query_select_last_activites_vs_savoirfaire_sql()

    for chunk in pd.read_sql_query(select_last_activites_vs_savoirfaire_sql, conn, chunksize=10000):
       chunk.to_csv(os.path.join(filepath_last_activites_vs_savoirfaire), index = True, index_label = "INDEX", mode='a',sep=';',encoding='utf-8')

    print(" --- select_last_activites_vs_savoirfaire_sql : OK")
